    ...
    for (const auto& entry : freq_pts) 
    {
        ...
        for (int i = 0; i < points.size(); i++) 
        {
            for (int j = i + 1; j < points.size(); ++j) 
            {
                ...
                if (n1.first >= 0 && n1.first < data.size() && n1.second >= 0 && n1.second < data[0].size()) 
                {
                    if (data[n1.first][n1.second] != '#') 
                    {
                ...
                if (n2.first >= 0 && n2.first < data.size() && n2.second >= 0 && n2.second < data[0].size()) 
                {
                    if (data[n2.first][n2.second] != '#') 
                    {
