...
static int compare(int page1, int page2, LIST **data)
{
    NODE *np = data[page1]->head->next;
    while (np != data[page1]->head)
    {
        if (np->data == page2)
        {
            return 1;
...
static void topoSort(int *update, int size, LIST **data)
{
    int swapped;
    do
    {
        ...
        {
            if (compare(update[i], update[i + 1], data))
            {
                int temp = update[i];
                update[i] = update[i + 1];
                update[i + 1] = temp;
